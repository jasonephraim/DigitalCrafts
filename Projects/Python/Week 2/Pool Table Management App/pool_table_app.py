...
def table_view():
    ...
    print(color.UNDERLINE + "Table Number", "Status",
          "\tStart Date and Time", "\t\tTotal Playtime" + color.END)
    for table in table_list:
        ...
        if table["start_date_time"] == "":
            ...
            print("Table {0} {1}".format(
                table["table_number"], " - ", table_status))
        # If there is a start time, set to occupied
            ...
            time_in_date_time = datetime.datetime.strptime(
                table_start_datetime, "%m/%d/%y %H:%M:%S")
            time_difference = datetime.datetime.now() - time_in_date_time
            minutes_played = math.floor(time_difference.total_seconds() / 60)
            print(color.RED + "Table {0} {1}\t Start: {2}\t Minutes Played: {3}".format(
                table["table_number"], table_status, table_start_datetime, minutes_played) + color.END)  # find way to make JSON display prettier data
...
def table_rent():
    ...
    if table_dictionary["start_date_time"] == "":
        table_dictionary["start_date_time"] = datetime.datetime.now().strftime(
            "%m/%d/%y %H:%M:%S")
        print("Pool table {0} rented out".format(
            table_dictionary["table_number"]))
    else:
        print("Pool table {0} is currently occupied".format(
            table_dictionary["table_number"]))
...
def table_end_rent():
    ...
    while True:
        ...
        if table_dictionary["start_date_time"] != "":       # ie Table is occupied
            ...
            update_pool_table_history_json_file(
                table_dictionary, table_history_file_string)
            ...
            print("Pool table {0} is not currently occupied".format(
                table_dictionary["table_number"]))
